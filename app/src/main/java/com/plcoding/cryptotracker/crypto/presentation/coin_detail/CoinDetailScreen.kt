...
import com.plcoding.cryptotracker.crypto.presentation.coin_list.models.toDisplayableNumber
    ...
    if (state.isLoading) {
        ...
        Column(
            ...
            FlowRow(
                ...
                val absoluteChangeUsd =
                    (coin.priceUsd.value * (coin.changePercent24Hr.value / 100)).toDisplayableNumber()
                val changeIsPositive = absoluteChangeUsd.value > 0.0
                val trendingContentColor = if (changeIsPositive) {
                ...
                InfoCard(
                    ...
                    formattedText = "$ ${absoluteChangeUsd.formatted}",
                    icon = if (changeIsPositive) ImageVector.vectorResource(R.drawable.trending)
                    else ImageVector.vectorResource(R.drawable.trending_down),
                    contentColor = trendingContentColor
                )
            }

            AnimatedVisibility(visible = coin.coinDataPoints.isNotEmpty()) {
                ...
                val amountOfVisibleDataPoint = if (labelWidth > 0) {
                    // TODO: change the value hardcoded
                    ((totalChartWidth - 2.5 * labelWidth) / labelWidth).toInt()
                } else {
                    0
                }
                val startIndex =
                    (coin.coinDataPoints.lastIndex - amountOfVisibleDataPoint).coerceAtLeast(0)
                LineChart(
                    dataPoints = coin.coinDataPoints,
                    style = ChartStyle(
                    ...
                    visibleDataPointsIndices = startIndex..coin.coinDataPoints.lastIndex,
                    unit = "$",
                    modifier = Modifier
                        .fillMaxWidth()
                        .aspectRatio(16 / 9f)
